import { app, BrowserWindow, ipcMain, shell, session, systemPreferences, protocol, net, nativeImage } from 'electron';
...
const WHISPER_BIN = process.env.NOTE_TAKER_WHISPER_BIN || 'whisper-cli';
...
const WHISPER_PROMPT = process.env.NOTE_TAKER_WHISPER_PROMPT ||
...
async function streamChat({ messages, tools, options, token, onProgress }) {
  ...
  try {
    ...
    if (token?.cancelled || err?.name === 'AbortError') throw new FormatCancelled();
    throw err;
  }
...
ipcMain.handle('notes:list', async () => {
  ...
  notes.sort((first, second) => (second.updatedAt || 0) - (first.updatedAt || 0));
  return notes;
});
...
ipcMain.handle('notes:create', async (_event, title) => {
  ...
  await fs.writeFile(join(notesDir(), `${note.id}.json`), JSON.stringify(note, null, 2));
  return note;
});
...
ipcMain.handle('audio:transcribe', async (_event, wavBuffer, opts = {}) => {
  ...
  const base = join(os.tmpdir(), `note-taker-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`);
  ...
  const carryover = typeof opts.carryover === 'string'
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      throw new Error(
        `Could not find "${WHISPER_BIN}". Install it with: brew install whisper-cpp`
      );
    }
    throw new Error(`whisper-cli failed: ${err.stderr?.toString() || err.message}`);
  } finally {
...
// note-image:// URLs.
ipcMain.handle('llm:format', async (event, { transcript, existing, context, images }) => {
  ...
  try {
    ...
    const preSearchResults = hasContext ? await preSearchFromContext(context) : [];
    ensureLive();
    ...
    const imageBase64 = hasImages
      ? await Promise.all(images.map(async (img) => (await fs.readFile(img.path)).toString('base64')))
      : [];
    ...
    const parts = [];
    if (hasContext) {
      parts.push(`Background context (for your interpretation only, do NOT echo into the notes):\n\n${context.trim()}`);
      parts.push('---');
    }
    ...
    if (hasImages) {
      const lines = ['Attached images (place each one where it fits best using ![caption](image:N)):'];
      imageDescriptions.forEach((desc, index) => {
        lines.push(`[${index + 1}] ${desc || 'image (no description available)'}`);
      });
    ...
    if (existing && existing.trim()) {
      ...
      parts.push('---');
      parts.push(hasTranscript ? `New transcript to merge:\n\n${transcript}` : '(No new transcript — just place the attached images appropriately in the existing notes.)');
    } else {
      parts.push(hasTranscript ? `Transcript:\n\n${transcript}` : '(No transcript — produce a brief note framed around the attached images.)');
    }
    parts.push(hasImages
    ...
    if (hasImages) {
      ...
      for (let index = 0; index < images.length; index++) {
        if (!out.includes(images[index].url)) {
          const caption = imageDescriptions[index] || `attached image ${index + 1}`;
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'A concise search query, like what you would type into a search engine.',
        },
      },
      required: ['query'],
    },
...
async function executeToolCall(name, rawArgs) {
  ...
  const query = String(args?.query || '').trim().slice(0, 250);
...
async function webSearch(query, maxResults = 5) {
  ...
  const results = [];
  const resultPattern = /<a\s+[^>]*class="[^"]*result__a[^"]*"[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>[\s\S]*?<a\s+[^>]*class="[^"]*result__snippet[^"]*"[^>]*>([\s\S]*?)<\/a>/gi;
  let match;
...
ipcMain.handle('llm:check', async () => {
  try {
    ...
    const has = models.models?.some((model) => model.name.startsWith(OLLAMA_MODEL));
